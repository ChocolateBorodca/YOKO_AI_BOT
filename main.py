...
# Твой личный проверенный ID для гарантированного безлимита
...
def get_chat_history(user_id, limit=6):
    ...
    for role, content in reversed(rows):
        history.append({"role": str(role[0]), "content": str(content[1])})
    return history
...
if __name__ == '__main__':
    init_db()
threading.Thread(target=lambda: HTTPServer(('0.0.0.0', 10000), Health).serve_forever(),
daemon=True).start()
if TELEGRAM_TOKEN:
